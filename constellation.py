...
def constellation():
    ...
    if not os.path.exists("角色命之座图标"):  # 判断是否存在文件夹，如不存在，继续
        os.mkdir("角色命之座图标")  # 新建文件夹

    for i in range(0, len(chara_name_list)):
        for m in range(1, 7):
            URL = "https://bbs.hoyolab.com/hoyowiki/picture/character/" + chara_name_list[i] + "/constellation_" + str(
                m) + ".png"  # 网址
            ...
            print("\n正在获取角色", chara_name_list[i], "的第" + after_name + "层命之座图标……")  # 提示信息
            ...
            doc_name = chara_name_list[i] + "·命之座第" + after_name + "层.png"  # 文件名称
            # 以二进制形式(因为是图片)的写入权限新创建(如已有，则覆盖其内容)一个 角色名.png 文件
            spritePage = open("角色命之座图标\\" + doc_name, "wb")
            # 向文件 角色名.png 写入变量 webPage_info 中的内容(获取到的图片的二进制的信息码)
            spritePage.write(webPage_info)
            spritePage.close()  # 将文件保存并关闭
            print("角色", chara_name_list[i], "的第" + after_name + "层命之座图标", doc_name, "保存成功！")  # 提示信息
